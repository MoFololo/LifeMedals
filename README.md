...
用户说一句话（比如"明天晚上10点前做两道LeetCode Medium"），或上传邮件、syllabus、活动海报等图片，AI 从中生成一份可编辑的"任务契约"（简短标题、任务说明、截止时间、所属勋章）；用户确认后去执行，完成后提交证据（截图/照片），AI 核验通过后获得对应勋章的经验值，并存入可回顾的成就 Library。图片生成的任务会保留压缩后的来源图，方便之后在任务详情中回看上下文。

## 核心产品闭环

```
说一句话
   → AI生成任务契约（简短标题/任务说明/截止时间/所属勋章，可编辑）
   → 用户确认
   → 执行任务
   → 提交证据
   → AI核验
...
- `TaskContract`：标题、任务说明、截止时间、内部锁定的核验依据、兼容旧版本的可选照片计划元数据、可选的任务来源图片、所属勋章、XP 奖励和状态。当前提交统一支持任意 1–5 张照片。
...
- 自然语言输入 → AI 生成任务契约 → 用户可编辑（简短标题/任务说明/截止时间/所属勋章）→ 确认
...
生成任务契约时，中文标题限制为 12 个字，英文标题限制为 8 个单词；详细输入会被整理到独立任务说明中。证据提交统一支持任意 1–5 张照片。iOS 支持相机、PhotosPicker 与文件选择；macOS 额外支持拖放和 `⌘V`。照片可在提交前预览删除；同一次提交的照片共享批次 ID，在历史区合并为一行。客户端先把原图转换为最长边不超过 1800 px、单张不超过约 1 MB 的 JPEG 副本，再写入 `Evidence.imageData` 的 SwiftData 外部存储；原始照片不复制进应用。核验失败时，整批本地记录保持 `Pending Verification`，可从任务详情重试。
